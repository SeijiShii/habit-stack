...
7. **作者を応援（任意）** — 継続サマリ閲覧時など満足ピークで、非ブロッキングに「💛 100円で応援」ワンタップ投げ銭（Stripe 単発、課金時のみ Google ログイン必須）。
8. **匿名 → アカウント連携** — 初回は匿名（IndexedDB に local-first 保存）で 0 タップ実行。デバイス間同期・課金のタイミングでのみ Google ログインを段階的に要求し、ローカルデータを引き継ぐ。
