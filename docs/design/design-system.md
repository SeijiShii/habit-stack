...
- [ ] Step 3 スタイル基盤適用（トークン → テーマ / 基本コンポーネント / ブランドマーク scaffold）— 実装フェーズ（Vite プロジェクト作成後）で実施
- [ ] Step 4 視覚レビュー — 画面実装後に `/flow:design --review-only`

> 本 SoT は `--system-only` で生成。コードベース（Vite プロジェクト）が未作成のため、トークン適用・視覚レビューは Phase 2/3 で実施。feature 設計は本 SoT のトークンを参照する。
