# バグ修正 #C20260616-001 ドキュメントインデックス

**issue / slug**: C20260616-001 / set-data-loss-after-login
**重大度**: high
**実施日**: 2026-06-16
**状態**: 実装完了（unit 248 green / typecheck clean）

<!-- auto-generated-start -->

## ファイル一覧
| 番号 | ファイル | 種別 | 最終更新 |
|---|---|---|---|
| 000 | `000_調査レポート.md` | 調査 | 2026-06-16 |
| 001 | `001_ROOT_CAUSE.md` | 根本原因（5 Whys） | 2026-06-16 |
| 002 | `002_FIX_PLAN.md` | 修正計画 | 2026-06-16 |
| 003 | `003_REGRESSION_TEST.md` | リグレッションテスト | 2026-06-16 |
| 004 | `004_POSTMORTEM.md` | Postmortem | 2026-06-16 |
| 101 | `101_FIX_IMPL_REPORT.md` | 実装レポート | 2026-06-16 |
| 102 | `102_FIX_UNIT_TEST_REPORT.md` | 単体テストレポート | 2026-06-16 |
| — | `README.md` | 概要 | 2026-06-16 |

## 関連
- 親 INDEX: `../INDEX.md`
- 基準 SPEC: `../001__shared_auth_SPEC.md`
- 起点クレーム: `../claim_C20260616-001_20260616_set-data-loss-after-login/001_TRIAGE.md`
- 回帰起点: `../revise_R20260615-001_20260615_account-switch-stop-sync/`

<!-- auto-generated-end -->

<!-- user-edit-start -->
<!-- user-edit-end -->
