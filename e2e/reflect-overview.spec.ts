...
test("E2E-OV-01: 継続ナビ → ふりかえり総覧（ドロップダウンのみ）→ セット選択で個別サマリへ", async ({
  page,
}) => {
  ...
  // 総覧の初期表示は「セットを選ぶ」ドロップダウンのみ（全セット一覧は出さない、R20260614 再設計）
  await expect(page.getByLabel("セットを選ぶ")).toBeVisible();
  await expect(page.locator("details")).toHaveCount(0);

  // ドロップダウンで個別サマリへ遷移（継続率 + 活動の記録の画面）
