...
test("E-LOGIN: 計時中にログイン画面(/account)へ遷移するとセッション終了", async ({
  page,
}) => {
  const runUrl = await startTimingOnSecondItem(page);

  // ログイン/アカウント画面へ遷移 → LoginEndGuard が進行中セッションを終了
  await page.goto("/account");
  await expect(page).toHaveURL(/\/account/);
  // 終了処理(endInProgressNow)の完了を待つ
  await page.waitForTimeout(500);

  // 実行画面に戻ると計時は復元されず「開始」状態（= 終了済み）
  await page.goto(runUrl);
  await expect(page.getByRole("button", { name: "開始" })).toBeVisible();
  await expect(page.getByTestId("current-item")).toHaveCount(0);
});
