...
const ach = (date: string, n = 1): AchievementDay => ({ date, itemDoneCount: n });
...
describe('summarize', () => {
  ...
  it('N1: 継続率 = 達成日/対象日', () => {
    const s = summarize(
      ['2026-06-01', '2026-06-02', '2026-06-05', '2026-06-08', '2026-06-09', '2026-06-10', '2026-06-04'].map((d) => ach(d)),
      dates,
    );
  ...
  it('N2: currentStreak = 末尾からの連続達成', () => {
    const s = summarize(['2026-06-08', '2026-06-09', '2026-06-10'].map((d) => ach(d)), dates);
    expect(s.currentStreak).toBe(3);
  });

  it('B2: 連続途切れ（1 日抜け）で streak リセット、エラー表現なし', () => {
    const s = summarize(['2026-06-08', '2026-06-10'].map((d) => ach(d)), dates);
  ...
  it('B1: 穴あき日（itemDoneCount=1）も達成カウント', () => {
    const s = summarize([ach('2026-06-10', 1)], enumerateDates('2026-06-10', '2026-06-10'));
